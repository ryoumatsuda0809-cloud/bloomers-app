...
export default function Quest2Page() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (data) {
        ...
        setSteps(data.quest2_steps ?? [])
        const firstIncomplete = (data.quest2_steps ?? [])
  ...
  const handleComplete = async (stepId: string) => {
    ...
    setSteps(updated)
    await updateQuestStepCompletion(projectId, 2, stepId, true)
      ...
      <div className="max-w-lg mx-auto space-y-6">
        ...
        {allCompleted && (
          <div className="bg-accent/30 border border-accent rounded-2xl p-5 text-center space-y-2">
            ...
            <p className="text-accent-foreground font-bold">クエスト2完了！</p>
            <button
              onClick={() => router.push('/')}
