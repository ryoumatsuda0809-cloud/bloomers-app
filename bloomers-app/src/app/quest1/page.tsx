...
import { updateStepCompletion } from '@/app/actions/setup'
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (data) {
        ...
        setSteps(data.setup_steps ?? [])
        const firstIncomplete = (data.setup_steps ?? [])
      ...
      <div className="max-w-lg mx-auto space-y-6">
        ...
        {allCompleted && (
          <div className="bg-accent/30 border border-accent rounded-2xl p-5 text-center space-y-2">
            ...
            <p className="text-accent-foreground font-bold">環境構築完了！</p>
            <button
              onClick={() => router.push('/')}
