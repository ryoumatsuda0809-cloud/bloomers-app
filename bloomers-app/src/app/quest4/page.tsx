...
export default function Quest4Page() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (data) {
        ...
        setSteps(data.quest4_steps ?? [])
        const firstIncomplete = (data.quest4_steps ?? [])
  ...
  const handleComplete = async (stepId: string) => {
    ...
    setSteps(updated)
    await updateQuestStepCompletion(projectId, 4, stepId, true)
      ...
      <div className="max-w-lg mx-auto space-y-6">
        ...
        {allCompleted && (
          <div className="bg-accent/30 border border-accent rounded-2xl p-5 text-center space-y-2">
            ...
            <p className="text-accent-foreground font-bold">クエスト4完了！</p>
            <button
              onClick={() => router.push('/')}
